    ...
    public class AccessMembershipProvider : MembershipProvider
    {
        ...
        public override void Initialize(string name, NameValueCollection config)
        {
            ...
            if (string.IsNullOrEmpty(config["description"]))
            {
                ...
                config.Add("description", "Membership DecemberAccessProvider Provider");
            }
        ...
        {
            //Normally, the userName is something the user chooses as their identifier. We are not using this
            //field in that way. Instead, we store a GUID on the userName field. 
            //We have users enter their email address when logging in and we can then look up the userName by
            //querying for the email address. 
            //The advantage to storing a GUID and using email as the username is that we can allow users to
            //change their email address at any point but changing your username is very difficult to do
            //with the way ASP.NET Membership is currently designed. 
            ...
            if (userId != null)
            {
                throw new ArgumentException("userId Parameter must be null for DecemberAccessProvider");
            }
        ...
        {
            if (!SecUtility.ValidateParameter(ref emailAddress,
                                               true,
                                               true,
                                               false,
                                               255))
            ...
            //We ask users to enter their email address in the UserName field when logging in. 
            //The database has a GUID for the userName, so we need to query the database using
            //the email address to retrieve this GUID. That's what tne next chunk of code does. 
            string username = this.GetUserNameByEmail(emailAddress);
